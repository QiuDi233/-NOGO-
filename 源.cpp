...
//判断边界
inline bool inBorder(int x, int y)//南希
{
...
//判断一个位置四周的在边界内的位置上有几个col色的棋子  南希
...
int eyelist[10][10] = { 0 };
//判断是否是电脑人工智能已经形成的眼，目的是最后在把自己形成的眼填上  南希
void eye()
{
	for (int x = 0; x < 9; x++)
		for (int y = 0; y < 9; y++)
...
//选点,每一个新的手数把valuepoint归零  评估每个点的value 南希
...
void blockopponenteye()//堵住对方将要造出的眼  南希
{
	for (int i = 0; i < 9; ++i) {
		for (int j = 0; j < 9; ++j) {
